...
# 2021/11/27 xingaofeng 修复开始节点是可压缩层，且是占位符的bug
class LegoDNNBlock(nn.Module):
    ...
    def __init__(self, original_model, num_in_block, legodnn_graph, block_info, is_empty=False):
        ...
        # 处理输入节点是module且是占位符的情况：不从原始模型中钩出该层，将将其设置为nn.Sequential()
    ...
    def _handle_matmul(self, tensor_list):
        assert len(tensor_list)==2
        z = torch.matmul(tensor_list[0], tensor_list[1])        
        return z
    ...
    def _handle_squeeze(self, tensor_list, node):
        x = tensor_list[0]
        auxiliary = node._auxiliary
        in_shape = auxiliary.get('in_shape') # 一定比out_shape长度多1
        out_shape = auxiliary.get('out_shape')  
        squeeze_dim = len(out_shape)
        for i, num in enumerate(out_shape):
            if in_shape[i] != num:
                squeeze_dim = i
                break
        return x.squeeze(squeeze_dim) 

    def _handle_expand_as(self, tensor_list, node):
        auxiliary = node._auxiliary
        in_shape = auxiliary.get('in_shape')
        out_shape = copy.deepcopy(auxiliary.get('out_shape'))
    ...
    def _handle_view(self, tensor_list, node):
        ...
        auxiliary = node._auxiliary
        out_shape = copy.deepcopy(auxiliary.get('out_shape'))
        # print("out_shape start: {}".format(out_shape))
        # print("x.size: {}".format(x.size()))
        real_batch_size = int(list(x.size())[0])
        out_shape[0] = real_batch_size
        if reduce(lambda x,y:x*y,out_shape[1:]) != reduce(lambda x,y:x*y,list(x.size())[1:]):
            if len(out_shape) > 2:
                out_shape[1] = int(reduce(lambda x,y:x*y,list(x.size())[1:]) / reduce(lambda x,y:x*y,out_shape[2:]))
            else:
                out_shape[1] = reduce(lambda x,y:x*y,list(x.size())[1:])
        # print("out_shape end: {}".format(out_shape))
        ...
        for i, forward_record in enumerate(self.forward_order):
            ...
            # 如果当前组件是func类型 需要使用自定义函数处理
                ...
                if op_type in ['aten::add', 'aten::add_']:
                    ...
                    output_tensor = [] if len(input_tensor_list)==0 else self._handle_mul(input_tensor_list)
                elif op_type == 'aten::matmul':
                    output_tensor = self._handle_matmul(input_tensor_list)
                elif op_type == 'aten::squeeze':
                    output_tensor = self._handle_squeeze(input_tensor_list, node)
...
if __name__=='__main__':
    ...
    active1 = ReuseLayerActivation(get_module(model, 'decode_head'), device)
