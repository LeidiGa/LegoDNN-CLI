...
class BenchmarkGraphData:
    ...
    def dump(self) -> dict:
        from legodnn.third_party.nni.nas.benchmarks.constants import DATABASE_DIR
        return {
...
class BenchmarkExecutionEngine(BaseExecutionEngine):
    ...
    def query_in_benchmark(graph_data: BenchmarkGraphData) -> Tuple[float, List[float]]:
        ...
        if graph_data.benchmark == 'nasbench101':
            from legodnn.third_party.nni.nas.benchmarks.nasbench101 import query_nb101_trial_stats
            arch = None
            ...
            from legodnn.third_party.nni.nas.benchmarks.nasbench201 import query_nb201_trial_stats
            ...
            from legodnn.third_party.nni.nas.benchmarks.nds import query_nds_trial_stats
            dataset = graph_data.benchmark.split('-')[-1]
            return _convert_to_final_and_intermediates(
            ...
            # FIXME: not tested yet
            from legodnn.third_party.nni.nas.benchmarks.nlp import query_nlp_trial_stats
